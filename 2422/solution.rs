...
macro_rules! input {
    ...
    (@inner $stdin:ident; [($($t:ty),*); $size:literal]) => {{
...
fn main() {
    ...
    let do_not_mix_count = input!(usize);
    let do_not_mix = input!(Vec[do_not_mix_count; [2; usize]]);

    let mut do_not_mix_map = vec![HashSet::new(); count + 1]; // make it 1-indexed.

    for [i, j] in do_not_mix {
        let (min, max) = if i < j { (i, j) } else { (j, i) };
        do_not_mix_map[min].insert(max);
    }
    ...
    for i in 1..=count {
        for j in (i + 1)..=count {
            for k in (j + 1)..=count {
                let set = &do_not_mix_map[i];
                if set.contains(&j) || set.contains(&k) {
                    continue;
                }
                let set = &do_not_mix_map[j];
                if set.contains(&k) {
                    continue;
                }
                available += 1;
