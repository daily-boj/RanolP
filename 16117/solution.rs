...
fn main() {
    ...
    let mut real_map = Vec::with_capacity(height * 2);
    for _ in 0..height {
        real_map.push(vec![0u64; width*2]);
    ...
    let get_safe = |map: &[Vec<u64>], x: usize, y: usize| -> u64 {
        if (0..2*height).contains(&y) && (0..2*width).contains(&x) {
            map[y][x]
        } else {
            0
        }
    ...
    let mut dp = vec![vec![0u64; 2*width]; 2*height];
    
    for x in (0..2*width).rev() {
        for y in (0..2*height).rev() {
            let here = real_map[y][x];
            let best = [
                get_safe(&real_map, x + 1, y - 1) + get_safe(&dp, x + 2, y - 2),
                get_safe(&real_map, x + 2, y) + get_safe(&dp, x + 4, y),
                get_safe(&real_map, x + 1, y + 1) + get_safe(&dp, x + 2, y + 2)
