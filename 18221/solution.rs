...
fn main() {
    ...
    for y in 0..n {
        ...
        let list = input!(Vec[n; usize]);
        let two = list.iter().position(|&x| x == 2);
        let five = list.iter().position(|&x| x == 5);
        match (two, five) {
            (Some(two), Some(five)) => {
                min_y = y;
                max_y = y;
    ...
    let distance = (max_x - min_x) * (max_x - min_x) + (max_y - min_y) * (max_y - min_y);
    let students: usize = columns.iter().skip(min_x).take(max_x + 1 - min_x).sum();

    println!(
        "{}",
        if distance >= 25 && students >= 3 {
            1
        } else {
            0
        }
