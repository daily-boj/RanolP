...
fn main() {
    ...
    let money_uses = input!(Vec[n; usize]);

    let search = |k: &usize| {
        let k = *k;
        let mut left_withdraw_count = m as isize;
        let mut balance = 0;
        for &i in &money_uses {
            if i > k {
                return BinarySearch::Short;
            }
            if i > balance {
                balance = k;
                left_withdraw_count -= 1;
            }
            balance -= i;
        }
        if left_withdraw_count < 0 {
            BinarySearch::Short
        } else if left_withdraw_count == 0 && balance == 0 {
            BinarySearch::Exact
        } else {
            BinarySearch::Over
        }
    ...
    let mut lo: usize = *money_uses.iter().min().unwrap();
    let mut hi: usize = money_uses.iter().sum();
