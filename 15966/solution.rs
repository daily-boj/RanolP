...
fn main() {
    ...
    let numbers = input!(Vec[count; u32]);
    let mut numbers_for_search = HashMap::with_capacity(count);

    for (index, value) in numbers.iter().enumerate() {
        if !numbers_for_search.contains_key(&value) {
            numbers_for_search.insert(value, Vec::new());
        }
        numbers_for_search.get_mut(&value).unwrap().push(index);
    }

    let mut answer = 1;
    for (index, initial) in numbers.iter().enumerate() {
        let mut length = 1;
        let mut begin = index;
        for i in 1.. {
            let values = if let Some(values) = numbers_for_search.get(&(initial + i)) {
                values
            } else {
                break;
            };
            match values.binary_search(&begin) {
                Ok(_) => { /* can't */ }
                Err(index) => {
                    if index < values.len() {
                        begin = values[index];
