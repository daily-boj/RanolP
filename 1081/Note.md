...
임의의 자연수 {@ var: m @}에 대해
{@ math: S(m + k) = S(m) + D(m) + D(m + 1) + \cdots + D(m + k - 1) + D(m + k) @}이므로
...
{@ math: S(10^k - 1) = 45 \times 10^{k - 1} + 10 \times S(10^{k - 1} - 1) (1 \gt k) @}
...
슬프게도 {@ math: S(a_0) + S(a_1 10) + S(a_2 10^2) + \cdots \eq S(a_0 + a_1 10 + a_2 10^2) @}가 아니다.
